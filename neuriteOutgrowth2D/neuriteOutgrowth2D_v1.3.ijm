...
#@ Integer (value = 0, label="Median Filter radius to apply on Mask (0 will not apply filter ):") medianFilterRadius
#@ boolean (label="save temporary images?", value = false) tmpImagesFlag
#@ boolean (label="Quantify on Signal Image (true -> qunatif on signal; false -> quantif on Mask)?", value = false) quantifOnSignal
...
function quantifications(image1,image2,stepSize,nSteps,savingDir){
	//print(image1);print(image2);print(stepSize);print(nSteps);print(savingDir);exit("error message");
	...
	for (i = 0; i < nSteps; i++) {
		...
		run("Intensity Measurements 2D/3D", "input="+image1+" labels=tmp mean numberofvoxels volume");
	...
	selectWindow(image1+"-intensity-measurements");run("Close");
